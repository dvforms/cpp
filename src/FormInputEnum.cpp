...
FieldType FormInputEnum::getType() const {
...
json FormInputEnum::generateSchema() const {
  ...
  if ( assoc ) {
    for ( const auto &item : options ) {
      opt[toKey( item.first )] = item.second;
    }
  } else {
    for ( const auto &item : options ) {
      opt.emplaceBack( std::make_shared<json>( item.second ) );
    }
  }
...
void dv::forms::from_json( const json &j, FormInputEnum &input, const dv::json::JSONPath &path ) {
  ...
  if ( opts ) {
    if ( opts->type() == dv::json::Type::OBJECT ) {
      for ( const auto &item : opts->objectIterator() ) {
        input.options.emplace( item.first, item.second->as<std::string>() );
      }
    } else if ( opts->type() == dv::json::Type::ARRAY ) {
      size_t i = 0;
      for ( const auto &item : opts->arrayIterator() ) {
        input.options.emplace( i++, item->as<std::string>() );
      }
    } else {
...
std::string FormInputEnum::toKey( const FormInputEnum::keyType &key ) const {
  return boost::get<std::string>( key );
}
