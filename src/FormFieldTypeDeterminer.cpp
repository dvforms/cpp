...
FormComponentPtr FormFieldTypeDeterminer::fromJSON( const json &j, FormGeneratorPtr &form, const dv::json::JSONPath &path ) {
  ...
  if ( !type ) {
    ...
    if ( x == "repeating" ) {
      ...
      rt = dv::json::JSONSerialiser<FormInput>::from_json( j, form->create<FormInputNumber>(), path );
    } else if ( x == "enum" ) {
      ...
      rt = dv::json::JSONSerialiser<FormInput>::from_json( j, form->create<FormInputDecimal>(), path );
    } else {
